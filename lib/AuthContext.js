...
import { supabase, getProfile, updateProfile, signOut } from './supabase';
import { api } from './api';
...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    // Safety net: if getSession() hangs (broken Supabase lock state), unblock after 5s
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(
      async (_event, session) => {
        ...
        setUser(session?.user ?? null);
        if (session?.user) {
          try { await loadProfile(session.user.id); } catch (_) { /* non-blocking */ }
          try {
