...
async function getAuthHeader() {
  try {
    ...
    // getSession() timed out or failed — read token directly from localStorage
    try {
...
export const api = {
  ...
  wallet: {
    get: () => apiCall(`/api/wallet?_=${Date.now()}`, { cache: 'no-store' }),
    initiateTopup: (packageId) => apiCall('/api/wallet/topup/initiate', { method:'POST', body:JSON.stringify({packageId}) }),
    confirmTopup: (sessionId,pkg) => apiCall('/api/wallet/topup/confirm', { method:'POST', body:JSON.stringify({session_id:sessionId,package_id:pkg}) }),
