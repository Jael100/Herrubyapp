...
export default function WalletScreen(){
  const { balance, addCredits, deductCredits } = useWallet();
  const onBalanceChange = (fn) => {
    const newBal = fn(balance);
    if (newBal > balance) addCredits(newBal - balance);
    else deductCredits(balance - newBal);
  };
  const [wView,setWView]=useState("overview");
  const [redeemed,setRedeemed]=useState({});
  const [showTopUp,setShowTopUp]=useState(false);
  const [showGift,setShowGift]=useState(false);
  const [txHistory,setTxHistory]=useState([{date:"Mar 10",desc:"Brain & Body Vitality enrolled",credits:-4,icon:"🧠",color:C.teal},{date:"Mar 1",desc:"Q1 credits from employer",credits:+10,icon:"💼",color:C.gold}]);
  ...
  const SOURCE_ICONS={employer:"💼",self:"💳",gift:"🎁"};
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(20,10,10,0.65)",zIndex:600,display:"flex",alignItems:"flex-end",justifyContent:"center"}} onClick={()=>{setShowTopUp(false);setTopupDone(false);setSelPkg(null);}}>
      <div onClick={e=>e.stopPropagation()} style={{background:C.cream,borderRadius:"28px 28px 0 0",width:"100%",maxWidth:430,padding:"30px 26px 54px",maxHeight:"90vh",overflowY:"auto"}}>
        ...
        {!topupDone?<div>
          ...
          <Btn onClick={()=>{if(selPkg){onBalanceChange(b=>b+selPkg.credits);setTxHistory(h=>[{date:"Today",desc:`Top-up: ${selPkg.label} (${selPkg.credits} credits)`,credits:+selPkg.credits,icon:"💳",color:C.indigo},...h]);setTopupDone(true);}}} disabled={!selPkg} s={{width:"100%",padding:"17px 0",fontSize:F.lg,marginTop:8}}>{selPkg?`Pay $${selPkg.price} ✦`:"Select a Package"}</Btn>
  ...
  if(showGift){return(
    <div style={{position:"fixed",inset:0,background:"rgba(20,10,10,0.65)",zIndex:600,display:"flex",alignItems:"flex-end",justifyContent:"center"}} onClick={()=>{setShowGift(false);setGiftSent(false);}}>
      <div onClick={e=>e.stopPropagation()} style={{background:C.cream,borderRadius:"28px 28px 0 0",width:"100%",maxWidth:430,padding:"30px 26px 54px",maxHeight:"90vh",overflowY:"auto"}}>
        <div style={{width:48,height:5,background:"#DDD",borderRadius:3,margin:"0 auto 24px"}}/>
        {!giftSent?<div>
          ...
          <Btn onClick={()=>setGiftSent(true)} disabled={!giftEmail||!giftPkg} s={{width:"100%",padding:"16px 0",fontSize:F.lg,background:C.gold}}>Send Gift ${giftPkg?.price||""} ✦</Btn>
        </div>:<div style={{textAlign:"center",padding:"20px 0"}}><div style={{fontSize:56,marginBottom:20}}>🎁</div><Serif s={{fontSize:F.xxl,color:C.slate,display:"block",marginBottom:12}}>Gift sent!</Serif><Sans s={{fontSize:F.md,color:C.muted,display:"block",lineHeight:1.8,marginBottom:22}}>{giftEmail} will receive their {giftPkg?.label} by email. What a beautiful thing to do. 💕</Sans><Btn onClick={()=>{setShowGift(false);setGiftSent(false);}} s={{width:"100%",padding:"16px 0",fontSize:F.lg}}>Done ✦</Btn></div>}
      </div>
    </div>
  );}
