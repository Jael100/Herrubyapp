            ...
            <div key={cat.id} style={{background:"white",borderRadius:24,overflow:"hidden",boxShadow:"0 3px 14px rgba(0,0,0,0.06)",border:`2px solid ${isOpen?cat.color:"transparent"}`,transition:"border 0.25s"}}>
              ...
              {isOpen&&<div style={{borderTop:`1px solid ${C.faint}`,padding:"4px 22px 22px"}}>
                {view==="live"&&content.map((s,i)=><div key={i} style={{paddingTop:18,borderTop:i>0?`1px solid ${C.faint}`:"none"}}>
                  ...
                  {registered[cat.id+i]?<div style={{background:C.sagePale,borderRadius:12,padding:"13px 18px",marginTop:10,textAlign:"center"}}><Sans s={{color:C.sage,fontWeight:600,fontSize:F.md}}>✓ Registered</Sans></div>:<Btn onClick={()=>setRegistered(r=>({...r,[cat.id+i]:true}))} s={{padding:"12px 24px",fontSize:F.md,background:cat.color,marginTop:10}}>Register Free →</Btn>}
                </div>)}
                {view==="library"&&content.map((item,i)=><div key={i} style={{paddingTop:18,borderTop:i>0?`1px solid ${C.faint}`:"none"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:10}}>
                    ...
                    <button onClick={()=>setSaved(sv=>({...sv,[cat.id+i]:!sv[cat.id+i]}))} style={{background:"none",border:"none",cursor:"pointer",fontSize:26,color:saved[cat.id+i]?C.ruby:"#DDD",flexShrink:0,paddingTop:4}}>{saved[cat.id+i]?"♥":"♡"}</button>
                  </div>
